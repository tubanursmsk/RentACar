...
export class StepPaymentComponent implements OnInit {
  ...
  cancelThreeDS(): void {
    ...
    // Frontend'i hemen kapat, kullanıcı bekletme
    this.threeDSHtml.set(null);
    ...
    this.paymentService.cancelPending(conversationId).subscribe({
      ...
      error: () => {
        // Backend hatası olsa bile frontend'i temizle
        this.cancelling.set(false);
        this.currentConversationId.set(null);
        this.wizard.reset();
        this.bookingState.clear();   // ⭐ YENİ
        ...
        this.paymentService.initThreeDS({
          ...
          error: (err) => {
            ...
            this.submitError.set(
              err.error?.message ||
              'Ödeme sistemi cevap vermiyor. Rezervasyon "Rezervasyonlarım" sayfanızda görünecek.'
            );
          }
        });
      },
